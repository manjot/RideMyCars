...
files = [
    'welcome.blade.php', 'about.blade.php', 'pricing.blade.php', 
    'ride.blade.php', 'rent.blade.php', 'hire-driver.blade.php', 
    'login.blade.php', 'signup.blade.php', 'terms.blade.php', 'privacy.blade.php'
]

# Ensure components directory exists
os.makedirs(os.path.join(views_dir, 'components'), exist_ok=True)

# The unified layout content
...
with open(os.path.join(views_dir, 'components', 'layout.blade.php'), 'w') as f:
...
for filename in files:
    ...
    if main_match and main_end_match:
        # Extract everything between <main...> and </main> (inclusive of the tags)
        start_idx = main_match.start()
        end_idx = main_end_match.end()
        main_content = content[start_idx:end_idx]
        
        title_match = re.search(r'<title>(.*?)—', content)
        title = 'RideMyCars'
        if title_match:
            title = title_match.group(1).strip() + ' — RideMyCars'
            
        new_content = f"<x-layout>\n    <x-slot:title>{title}</x-slot>\n\n    {main_content}\n\n</x-layout>"
