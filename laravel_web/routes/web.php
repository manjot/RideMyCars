...
Route::post('/api/otp/send', function (\Illuminate\Http\Request $request) {
    ...
    if (!empty($phone)) {
        ...
        // Store OTP in Cache with exact 5-minute validity
        \Illuminate\Support\Facades\Cache::put('otp_phone_' . $formattedPhone, $otp, now()->addMinutes(5));
        if ($rawCleanPhone !== $formattedPhone) {
            \Illuminate\Support\Facades\Cache::put('otp_phone_' . $rawCleanPhone, $otp, now()->addMinutes(5));
        }
        ...
        $request->session()->put('otp_phone_expires', now()->addMinutes(5)->timestamp);
...
Route::post('/api/otp/verify', function (\Illuminate\Http\Request $request) {
    try {
        ...
        if (!empty($phone)) {
            ...
            if ($cachedOtp && (string) $cachedOtp === $inputOtp) {
                ...
                if (!$user) {
                    ...
                    // Build user data with schema-safe column verification
                    $userData = [
                        'name' => $userName,
                        'phone' => $formattedPhone,
                        'email' => $userEmail,
                    ...
                    if (\Illuminate\Support\Facades\Schema::hasColumn('users', 'avatar') && $avatarPath) {
                        $userData['avatar'] = $avatarPath;
                    }
                    if (\Illuminate\Support\Facades\Schema::hasColumn('users', 'referral_code')) {
                        $userData['referral_code'] = null; // Auto-generated by User model booted()
                    }
                    if (\Illuminate\Support\Facades\Schema::hasColumn('users', 'referred_by')) {
                        $userData['referred_by'] = $referredBy;
                    }
                    if (\Illuminate\Support\Facades\Schema::hasColumn('users', 'referrer_id')) {
                    ...
                    if (\Illuminate\Support\Facades\Schema::hasColumn('users', 'terms_accepted')) {
                        ...
                        $userData['terms_version'] = '2026-08-23';
                    }
                    ...
                    if ($role === 'driver') {
                        try {
                            ...
                            \Illuminate\Support\Facades\Log::warning('Driver profile create warning: ' . $e->getMessage());
                        }
                    }
                } else {
            ...
            return response()->json([
                'success' => false,
                'error' => 'Invalid or expired OTP code. OTP is valid for 5 minutes.'
            ], 422);
        }
        ...
        \Illuminate\Support\Facades\Log::error('OTP Verify Exception: ' . $e->getMessage() . "\n" . $e->getTraceAsString());
        return response()->json([
            'success' => false,
...
Route::post('/set-country', function (\Illuminate\Http\Request $request) {
    ...
    $isAuto = $request->boolean('auto', false);
    \App\Services\CountryService::persistCountry($code, !$isAuto);
