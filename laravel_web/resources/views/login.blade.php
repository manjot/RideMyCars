<x-layout>
        ...
        <div class="w-full max-w-[440px] bg-white dark:bg-[#141414] rounded-3xl p-7 sm:p-9 shadow-xl border border-gray-200/80 dark:border-white/10 transition-colors">
            ...
            <div x-show="view === 'mobile'" x-transition.opacity.duration.300ms>
                ...
                <form action="#" method="POST" @submit.prevent="submitMobile()">
                    ...
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-2.5 text-center leading-relaxed">
                        By continuing, you agree to receive an SMS verification code from RideMyCars. Msg & data rates may apply. Reply STOP to cancel.
                    </p>
                </form>
                ...
                <div class="space-y-3">
                    <button type="button" id="google-login-btn" onclick="triggerGoogleAuth('customer')" class="w-full flex items-center justify-center gap-3 py-3.5 bg-gray-100 hover:bg-gray-200 dark:bg-white/5 dark:hover:bg-white/10 text-gray-900 dark:text-white font-semibold rounded-xl border border-gray-200/50 dark:border-white/5 transition-all duration-200 shadow-sm hover:shadow active:scale-[0.99] text-decoration-none cursor-pointer">
                        ...
                        <span id="google-login-btn-text">Continue with Google</span>
                    </button>
                    <a href="{{ route('auth.apple') }}" class="w-full flex items-center justify-center gap-3 py-3.5 bg-gray-100 hover:bg-gray-200 dark:bg-white/5 dark:hover:bg-white/10 text-gray-900 dark:text-white font-semibold rounded-xl border border-gray-200/50 dark:border-white/5 transition-all duration-200 shadow-sm hover:shadow active:scale-[0.99] text-decoration-none cursor-pointer">
    ...
    <script>
            ...
            if (window.Alpine && !window.__loginAppRegistered) {
                ...
                Alpine.data('loginApp', () => ({
                    ...
                    submitMobile() {
                        ...
                        let cleanMobile = (this.mobileNumber || '').replace(/[^\d]/g, '');
                        if (cleanMobile.startsWith('0') && this.selectedCountry && this.selectedCountry.dial !== '+1') {
                            cleanMobile = cleanMobile.substring(1);
                        }
                        const fullPhone = (this.selectedCountry ? this.selectedCountry.dial : '+233') + cleanMobile;
        ...
        function triggerGoogleAuth(role = 'customer') {
            ...
            const origText = btnText ? btnText.innerText : 'Continue with Google';

            if (btn) {
                btn.classList.add('opacity-75', 'pointer-events-none');
            }
            if (btnText) {
                btnText.innerText = 'Connecting to Google...';
            }
            ...
            try {
                const client = google.accounts.oauth2.initCodeClient({
                    ...
                    callback: (response) => {
                        if (response.code) {
                            if (btnText) btnText.innerText = 'Verifying account...';
                            ...
                            .then(data => {
                                if (data.status === 'success' && data.redirect) {
                                    if (btnText) btnText.innerText = 'Success! Redirecting...';
                                    window.location.href = data.redirect;
                                } else {
