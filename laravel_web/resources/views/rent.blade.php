...
<head>
    ...
    <title>Rent a Vehicle — RideMyCars</title>
    <!-- Fonts -->
...
<body class="font-sans antialiased bg-[#fafafa] text-gray-900 min-h-screen flex flex-col">
        ...
        <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex h-20 items-center justify-between">
                ...
                <div class="hidden lg:flex items-center gap-6">
                    ...
                    <a class="text-sm font-medium transition-colors text-gray-900 bg-gray-100 px-4 py-2 rounded-full" href="/rent">Rent Vehicle</a>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12"
        ...
        <!-- Header Text -->
        <div class="mb-10">
            <h1 class="text-4xl font-bold text-gray-900 mb-3 tracking-tight">Rent a Vehicle</h1>
            <p class="text-gray-500 text-lg" x-text="`${filteredVehicles.length} vehicles available — filter by type, price, and features.`"></p>
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-8">
            <div class="relative flex-1">
                ...
                <input x-model="search" type="text" placeholder="Search by make, model..." class="w-full pl-12 pr-4 py-3.5 bg-white border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all shadow-sm">
            </div>
            <button class="flex items-center justify-center gap-2 px-6 py-3.5 bg-white border border-gray-200 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 transition-all shadow-sm">
        ...
        <div class="flex flex-wrap gap-3 mb-10">
            <template x-for="category in categories" :key="category">
                <button 
                    ...
                    :class="selectedCategory === category ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-700 border-gray-200 hover:border-gray-300'"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <template x-if="filteredVehicles.length === 0">
                <div class="col-span-full text-center py-12">
                    <p class="text-gray-500 text-lg">No vehicles found matching your criteria.</p>
                </div>
            </template>

            <template x-for="vehicle in filteredVehicles" :key="vehicle.id">
                <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col group cursor-pointer">
                    <!-- Image -->
                    <div class="w-full h-48 bg-gray-100 rounded-xl mb-4 overflow-hidden relative">
                        ...
                        <!-- Badge -->
                        <div class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-gray-700 shadow-sm" x-text="vehicle.type"></div>
                    </div>
                    
                    <!-- Details -->
                    <div class="mt-auto">
                        <div class="flex justify-between items-start mb-1">
                            <h3 class="font-bold text-lg text-gray-900" x-text="`${vehicle.year} ${vehicle.make}`"></h3>
                            <div class="font-bold text-lg text-gray-900" x-text="`$${vehicle.daily_rate}`"><span class="text-sm font-normal text-gray-500">/day</span></div>
                        </div>
                        <p class="text-gray-500 text-sm font-medium mb-4" x-text="vehicle.model"></p>
                        <button class="w-full py-2.5 bg-gray-50 hover:bg-orange-50 text-gray-900 hover:text-orange-600 border border-gray-200 hover:border-orange-200 rounded-xl font-bold transition-colors">
                            View Details
                        </button>
                    </div>
                </div>
