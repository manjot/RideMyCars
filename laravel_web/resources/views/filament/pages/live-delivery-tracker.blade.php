<x-filament-panels::page>
    ...
    <div x-data="liveDeliveryTracker()" x-init="init()" class="space-y-6 w-full">
        
        <!-- Top Operations Status Banner -->
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-5 bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm gap-4">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-brand-500/10 text-brand-500 flex items-center justify-center font-bold text-lg shrink-0">
                    🚚
                </div>
                <div>
                    <h2 class="text-xl font-black text-gray-900 dark:text-white">Logistics Control Center — Live Delivery Tracker</h2>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Package Delivery Vertical • 15% Platform Fee / 85% Driver Payout Model</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 text-xs font-bold shrink-0">
                <div class="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-green-50 dark:bg-green-950/40 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-800/30">
                    ...
                    <span>Live GPS Polling (10s)</span>
                </div>
                <button @click="refreshStream()" :disabled="isRefreshing" class="px-5 py-2.5 bg-brand-500 hover:bg-brand-600 active:scale-95 text-white rounded-xl font-bold transition-all shadow-md flex items-center gap-2 cursor-pointer">
                    ...
                    <span x-text="isRefreshing ? 'Refreshing...' : 'Refresh Stream'"></span>
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start w-full">
            ...
            <!-- LEFT SIDE: ZONE 1 (Order Feed) & ZONE 2 (Order Details Desk) -->
            <div class="lg:col-span-5 space-y-6">
                
                <!-- ZONE 1: ACTIVE ORDER FEED -->
                <div class="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-white/10 p-5 shadow-sm space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[11px] font-bold text-gray-400 bg-gray-50 dark:bg-white/5 px-2 py-0.5 rounded-md">Zone 1</span>
                    </div>

                    <!-- Search Bar -->
                    <div class="relative">
                        <input x-model="search" type="text" placeholder="Search Order ID, Driver, Merchant..." class="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-xl text-xs text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        ...
                        <button @click="selectedFilter = 'IN-TRANSIT'" :class="selectedFilter === 'IN-TRANSIT' ? 'bg-blue-600 text-white font-bold' : 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-gray-200'" class="px-3 py-1.5 rounded-lg text-xs transition-colors">
                            In-Transit
                        </button>
                        <button @click="selectedFilter = 'DELAYED'" :class="selectedFilter === 'DELAYED' ? 'bg-rose-600 text-white font-bold' : 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-gray-200'" class="px-3 py-1.5 rounded-lg text-xs transition-colors">
                            Delayed
                        </button>
                        <button @click="selectedFilter = 'COMPLETED'" :class="selectedFilter === 'COMPLETED' ? 'bg-emerald-600 text-white font-bold' : 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-gray-200'" class="px-3 py-1.5 rounded-lg text-xs transition-colors">
                            Completed
                        </button>
                    </div>
                    ...
                    <div class="space-y-3 max-h-[340px] overflow-y-auto pr-1">
                        <template x-if="filteredOrders.length === 0">
                            <div class="p-6 text-center text-gray-400 text-xs italic">
                                No active delivery orders match your search.
                            </div>
                        </template>
                ...
                <!-- ZONE 2: ORDER DETAILS DESK -->
                <template x-if="selectedOrder">
                    <div class="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-white/10 p-6 shadow-sm space-y-6">
                        
                        <div>
                            <div class="flex items-center justify-between mb-3">
                                <div>
                                    <span class="text-xs font-bold text-brand-500 uppercase tracking-wider block">Zone 2 — Order Details Desk</span>
                                    <h3 class="text-2xl font-black text-gray-900 dark:text-white" x-text="selectedOrder.digital_receipt_code"></h3>
                                </div>
                                <div class="text-right bg-brand-50 dark:bg-brand-950/40 p-2.5 rounded-xl border border-brand-200 dark:border-brand-900/40">
                                    <span class="text-[10px] font-bold text-gray-400 block uppercase">Est. Arrival (ETA)</span>
                                    <span class="text-xl font-black text-brand-600 dark:text-brand-400" x-text="`${selectedOrder.estimated_minutes} min`"></span>
                                </div>
                            </div>

                            <!-- Delayed Warning Banner -->
                            <template x-if="selectedOrder.is_delayed">
                                <div class="p-4 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800/40 text-rose-800 dark:text-rose-300 flex items-center justify-between gap-3 shadow-sm">
                                    ...
                                    <button @click="openReassignModal()" class="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-extrabold shadow-sm shrink-0">
                                        RE-ASSIGN ORDER
                                    </button>
                                </div>
                            </template>
                        ...
                        <div class="border-t border-gray-100 dark:border-white/10 pt-4 space-y-3">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                <!-- SENDER -->
                                <div class="p-3.5 bg-gray-50 dark:bg-[#161616] rounded-xl border border-gray-100 dark:border-white/5 space-y-1">
                        ...
                        <div class="border-t border-gray-100 dark:border-white/10 pt-4 space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-900/30">
                                    15% Platform / 85% Driver Model
                                </span>
                            </div>

                            <div class="p-4 bg-gray-50 dark:bg-[#161616] rounded-xl border border-gray-100 dark:border-white/5 space-y-2 text-xs">
                                ...
                                <div class="flex justify-between font-semibold text-green-600 dark:text-green-400 pt-1.5 border-t border-gray-200 dark:border-white/10">
                                    ...
                                    <span class="font-black text-sm" x-text="`GH₵ ${selectedOrder.driver_payout_85}`"></span>
                                </div>
                        ...
                        <!-- 3. Proof of Delivery (PoD) & Dispute Support -->
                        <div class="border-t border-gray-100 dark:border-white/10 pt-4 space-y-3">
                            <div class="flex items-center justify-between">
                                <h4 class="text-xs font-extrabold uppercase tracking-wider text-gray-400">PROOF OF DELIVERY (PoD)</h4>
                                <span :class="selectedOrder.pod.status === 'VERIFIED' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'" class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase">
                                    <span x-text="selectedOrder.pod.status"></span>
                                </span>
                            </div>

                            <div class="p-4 bg-gray-50 dark:bg-[#161616] rounded-xl border border-gray-100 dark:border-white/5 text-xs space-y-3">
                                <div class="flex items-center justify-between">
                                    <div>
                                    ...
                                    <button @click="openPodModal()" class="px-3.5 py-2 bg-gray-900 hover:bg-black text-white rounded-xl font-bold text-xs transition-colors shadow-sm">
                                        VIEW PROOF OF DELIVERY
                                    </button>
                                </div>

                                <template x-if="selectedOrder.pod.photo_url">
                                    <div class="flex items-center gap-3 pt-2 border-t border-gray-200 dark:border-white/5">
                                        <div class="w-12 h-12 rounded-lg bg-gray-200 overflow-hidden shrink-0 border">
                                            <img :src="selectedOrder.pod.photo_url" class="w-full h-full object-cover" alt="PoD Photo">
                                        </div>
                                        <div class="text-[11px]">
                                            <span class="font-bold text-gray-900 dark:text-white block">Recipient Signature Recorded</span>
                                            <span class="text-gray-500">Verified digitally on delivery</span>
                                        </div>
                                    </div>
                                </template>
                            </div>
                        ...
                        <div class="pt-2 flex items-center gap-3">
                            <button @click="openReassignModal()" class="flex-1 py-3 px-4 bg-brand-500 hover:bg-brand-600 text-white rounded-xl font-bold text-xs transition-all shadow-md text-center">
                                🔄 Re-Assign Order
                            </button>
                            <button @click="centerMapOnSelected()" class="py-3 px-4 bg-gray-100 dark:bg-white/10 hover:bg-gray-200 text-gray-900 dark:text-white rounded-xl font-bold text-xs transition-all text-center">
                                📍 Center Map
                            </button>
                        </div>
                    </div>
            ...
            <!-- RIGHT SIDE: ZONE 3 (Live Geospatial Map ~65% Width) -->
            <div class="lg:col-span-7">
                <div class="bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-white/10 p-5 shadow-sm space-y-4 sticky top-20">
                    <div class="flex items-center justify-between">
                    ...
                    <!-- Map Container -->
                    <div class="relative w-full h-[600px] rounded-xl overflow-hidden border border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-[#1a1a1a]">
                        <div id="live_tracker_map" class="w-full h-full"></div>

                        <!-- Floating Live Info Overlay -->
                        <template x-if="selectedOrder">
                            <div class="absolute bottom-4 left-4 right-4 bg-white/95 dark:bg-black/90 backdrop-blur-md p-4 rounded-xl border border-gray-200 dark:border-white/10 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                                <div>
