<x-layout>
    ...
    <div class="pt-24 pb-12 bg-gray-50 dark:bg-[#09090b] min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Incentive Program (Daily, Weekly, Monthly) -->
            <div x-data="driverIncentiveProgram(@js($incentivesData ?? []))" class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-3xl p-6 sm:p-8 shadow-sm mb-8">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-100 dark:border-white/10">
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <h2 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight">Driver Incentive Program</h2>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-white/10 text-xs font-semibold text-gray-700 dark:text-gray-300">
                            <span>📍</span>
                            <span x-text="data.driver_location?.display_location || 'India'"></span>
                            <span class="text-gray-400">•</span>
                            <span x-text="data.driver_location?.vehicle_type || 'Car'"></span>
                        </div>
                        <button type="button" @click="fetchIncentives()" :disabled="loading" class="p-2 rounded-xl bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/20 transition-all text-xs font-bold" title="Refresh Incentives">
                ...
                <div class="mt-8 pt-6 border-t border-gray-100 dark:border-white/10 grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div class="space-y-2 max-h-56 overflow-y-auto pr-1">
                            <template x-if="!data.reward_history || data.reward_history.length === 0">
                                <div class="py-6 text-center text-xs text-gray-400 bg-gray-50 dark:bg-white/5 rounded-2xl">
                                    No incentive rewards credited yet. Complete your first milestone to earn!
                                </div>
                            </template>

                            <template x-for="r in (data.reward_history || [])" :key="r.id">
                                <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 text-xs">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center font-bold">
                                            🎁
                                        </div>
                                        <div>
                                            <div class="font-bold text-gray-900 dark:text-white" x-text="r.incentive"></div>
                                            <div class="text-[11px] text-gray-400" x-text="r.date + ' • ' + r.trips"></div>
                                        </div>
                                    </div>
                    ...
                    <!-- Wallet Credits Box -->
                    <div class="p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 to-yellow-500/5 border border-amber-400/30 text-gray-900 dark:text-white flex flex-col justify-between">
                        <div>
                            ...
                            <div class="text-3xl font-black text-gray-900 dark:text-white mt-1">
                                <span x-text="'₹' + Number(data.wallet_summary?.total_incentive_earned || 0).toLocaleString()"></span>
                            </div>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">All rewards are instantly deposited into your driver wallet upon achieving ride targets.</p>
                        </div>

                        <div class="mt-4 pt-3 border-t border-amber-400/20 flex items-center justify-between text-xs">
                            <span class="text-gray-500 dark:text-gray-400">Current Balance:</span>
                            <span class="font-extrabold text-emerald-600 dark:text-emerald-400" x-text="'₹' + Number(data.wallet_summary?.balance || 0).toFixed(2)"></span>
                        </div>
                    </div>
                </div>
            </div>
