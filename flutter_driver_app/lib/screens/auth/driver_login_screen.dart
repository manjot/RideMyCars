...
enum DriverAuthMethod { phone, emailOtp, password }
...
class _DriverLoginScreenState extends State<DriverLoginScreen> {
  ...
  DriverAuthMethod _authMethod = DriverAuthMethod.phone;
  ...
  Future<void> _handleSendPhoneOtp() async {
    ...
    if (res['not_found'] == true || res['user_exists'] == false) {
      _showRegisterPrompt(
        ...
        message: 'No driver account found for $phone. Would you like to register as a driver now?',
        onRegister: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => DriverRegisterScreen(initialPhone: phone)),
          );
        },
      );
  ...
  Future<void> _handleVerifyPhoneOtp() async {
    ...
    final auth = Provider.of<AuthProvider>(context, listen: false);
    final success = await auth.verifyPhoneOtp(phone: phone, otp: otp, role: 'driver');
  ...
  Future<void> _handleSendEmailOtp() async {
    ...
    if (email.isEmpty || !email.contains('@')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter a valid driver email address'),
          backgroundColor: AppColors.danger,
        ),
      );
      return;
    ...
    if (res['not_found'] == true || res['user_exists'] == false) {
      _showRegisterPrompt(
        ...
        message: 'No driver account found with $email. Would you like to register as a driver now?',
        onRegister: () {
          Navigator.push(
            context,
  ...
  Future<void> _handleVerifyEmailOtp() async {
    ...
    final auth = Provider.of<AuthProvider>(context, listen: false);
    final success = await auth.verifyEmailOtp(email: email, otp: otp, role: 'driver');
  ...
  Future<void> _handleGoogleLogin() async {
    ...
    final proceed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text('Connect your Google Account to sign in to Driver Portal instantly:', style: TextStyle(color: AppColors.textMuted, fontSize: 13)),
    ...
    final success = await auth.loginWithGoogle(
      email: email,
      name: nameInput.text.trim().isNotEmpty ? nameInput.text.trim() : 'Google Driver',
  ...
  Future<void> _handleAppleLogin() async {
    ...
    final proceed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            const Text('Sign in with your Apple ID to access the Driver Portal:', style: TextStyle(color: AppColors.textMuted, fontSize: 13)),
            const SizedBox(height: 16),
            TextField(
              controller: emailInput,
              keyboardType: TextInputType.emailAddress,
    ...
    final success = await auth.loginWithApple(
      ...
      name: nameInput.text.trim().isNotEmpty ? nameInput.text.trim() : 'Apple Driver',
      role: 'driver',
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Register as Driver', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        _buildTabButton(
                          title: '📱 Phone OTP',
                          isSelected: _authMethod == DriverAuthMethod.phone,
                          onTap: () => setState(() => _authMethod = DriverAuthMethod.phone),
                        ),
                        _buildTabButton(
                          title: '✉️ Email OTP',
                          isSelected: _authMethod == DriverAuthMethod.emailOtp,
                          onTap: () => setState(() => _authMethod = DriverAuthMethod.emailOtp),
                        ),
                        _buildTabButton(
                          title: '🔑 Password',
                          isSelected: _authMethod == DriverAuthMethod.password,
                          onTap: () => setState(() => _authMethod = DriverAuthMethod.password),
                        ),
                      ],
                    ),
                  ),
                  ...
                  if (_authMethod == DriverAuthMethod.phone) ...[
                    Container(
                      decoration: BoxDecoration(
                        color: AppColors.surfaceDark,
                  ...
                  // --- 2. Email OTP Section ---
                  if (_authMethod == DriverAuthMethod.emailOtp) ...[
                    TextFormField(
                  ...
                  // --- 3. Password Section ---
                  if (_authMethod == DriverAuthMethod.password) ...[
                    TextFormField(
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: auth.isLoading
                            ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2.5, color: AppColors.backgroundDark))
                            : const Text('Sign In to Dashboard', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900)),
                      ),
                    ),
                  ],
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      const Text("New driver? ", style: TextStyle(color: AppColors.textMuted, fontSize: 13)),
                      GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: isLoading
                ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2.5, color: AppColors.backgroundDark))
                : const Text('Verify & Open Dashboard', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900)),
          ),
        ),
      ],
    );
